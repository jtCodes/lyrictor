...
// Adapted from web-fsr; see LICENSE.web-fsr. RGBA premultiplied canvas variant.
/*
...
void FsrEasuTapF(
    inout vec4 aC, // Accumulated color, with negative lobe.
...
)
{
    ...
    float dirX = lD - lB;
    dir.x += dirX * w;
    lenX = clamp(abs(dirX)/max(lenX, 0.00001),0.,1.);
    lenX *= lenX;
    len += lenX * w;
    ...
    float dirY = lE - lA;
    dir.y += dirY * w;
    lenY = clamp(abs(dirY) / max(lenY, 0.00001),0.,1.);
    lenY *= lenY;
    len += lenY * w;
}
...
)
{
    ...
    vec4 bC = FsrEasuCF(p0 + off.xw); float bL = bC.g + 0.5 *(bC.r + bC.b);
    vec4 cC = FsrEasuCF(p0 + off.yw); float cL = cC.g + 0.5 *(cC.r + cC.b);
    vec4 iC = FsrEasuCF(p1 + off.xw); float iL = iC.g + 0.5 *(iC.r + iC.b);
    vec4 jC = FsrEasuCF(p1 + off.yw); float jL = jC.g + 0.5 *(jC.r + jC.b);
    vec4 fC = FsrEasuCF(p1 + off.yz); float fL = fC.g + 0.5 *(fC.r + fC.b);
    vec4 eC = FsrEasuCF(p1 + off.xz); float eL = eC.g + 0.5 *(eC.r + eC.b);
    vec4 kC = FsrEasuCF(p2 + off.xw); float kL = kC.g + 0.5 *(kC.r + kC.b);
    vec4 lC = FsrEasuCF(p2 + off.yw); float lL = lC.g + 0.5 *(lC.r + lC.b);
    vec4 hC = FsrEasuCF(p2 + off.yz); float hL = hC.g + 0.5 *(hC.r + hC.b);
    vec4 gC = FsrEasuCF(p2 + off.xz); float gL = gC.g + 0.5 *(gC.r + gC.b);
    vec4 oC = FsrEasuCF(p3 + off.yz); float oL = oC.g + 0.5 *(oC.r + oC.b);
    vec4 nC = FsrEasuCF(p3 + off.xz); float nL = nC.g + 0.5 *(nC.r + nC.b);
    ...
    //  i j k l
    //    n o
    vec4 min4 = min(min(fC,gC),min(jC,kC));
    vec4 max4 = max(max(fC,gC),max(jC,kC));
    // Accumulation.
    vec4 aC = vec4(0);
    float aW = 0.;
...
void main()
{
    ...
    FsrEasuF(c, fragCoord.xy - vec2(0.5), con0, con1, con2, con3);
    fragColor = vec4(clamp(c.rgb, vec3(0.0), vec3(clamp(c.a, 0.0, 1.0))), clamp(c.a, 0.0, 1.0));
}
